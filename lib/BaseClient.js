...
const dict_last_bracket = require('./dict_last_bracket')
...
BaseClient.checkMessage = function(msg) {
  // takes in ecoded, return the encoded index of last bracket
  ...
  if (-1 !== idx) {
    try {
      const id = m.substring(0, idx);
      const data = dict_last_bracket(m.substring(idx + 1), 'utf8');
      return data + id.length + 1;
    } catch (err) { 
      return -1;
    }
